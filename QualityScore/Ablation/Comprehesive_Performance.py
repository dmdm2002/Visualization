...
def get_image_path(model, fold, ep):
    if fold == '1-fold':
        cls = 'B'
    else:
        cls = 'A'
    ...
    gen_path = glob.glob(f'Z:/2nd_paper/backup/GANs/ND/{model}/{fold}/test/{cls}/A2B/{ep}/*')
...
def metrics(model, fold, ep, score_names):
    zip_data = get_image_path(model, fold, ep)
    if fold == '1-fold':
        full_len = 2509
    else:
        full_len = 2277
...
if __name__ == '__main__':
    ...
    for i in range(len(models)):
        ...
        for fold in folds:
            # model, fold, ep
            mean_scores = metrics(model, fold, info['ckp'][i][fold], scores)
            ...
            os.makedirs('Z:/2nd_paper/backup/score/2023-06-20/image_quality', exist_ok=True)
            df.to_csv(f'Z:/2nd_paper/backup/score/2023-06-20/image_quality/{model}_{fold}.csv', index=False)
